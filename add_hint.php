                  ...
                  if ($result !== false) {
                    ...
                    while ($result !== false) {
                      if ($result['category'] == 3 || $category == 3 || $result['category'] == $category) {
                        $to = $result['email'];
                        $subject = "Nová nápověda k Předmajálesové hře - kolo $round_id \"$nickname\"";

                        $mail_body = "<!DOCTYPE html><html lang='en' style='font-family: Verdana, sans-serif; background: hsl(192, 15%, 10%); color: white;'><body style='font-family: Verdana, sans-serif; background: hsl(192, 15%, 10%); color: white;'>  <h1 style='text-shadow: 2px 1px hsl(302, 100%, 66%), -2px -1px hsl(182, 98%, 23%); font-size: 3rem;'>Ke kolu $nickname byla přidána nová nápověda</h1>  <h3>Stačí najít místo na obrázku, navštívit ho a vyfotit samolepku s logem majálesu do $end. To přece zvládne každý!!!  </h3>  <h3>Podrobnosti: <a href='https://majales.gyrec.cz/predmajalesovka/round.php?round=$new_round_id' style='transition: text-shadow 0.1s ease; color: hsl(179, 100%, 50%);'>https://majales.gyrec.cz/predmajalesovka/round.php?round=$new_round_id</a>  </h3>  <h3>PoZnávÁte tOTo MÍSto???????:</h3>";
